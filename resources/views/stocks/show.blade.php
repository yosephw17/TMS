        ...
        <div class="card-body">
            ...
            <div class="accordion" id="materialsAccordion">
                @foreach ($groupedMaterials as $materialId => $materialEntries)
                                    ...
                                    <table class="table table-sm table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($materialEntries as $index => $entry)
                                                <tr
                                                    class="{{ $entry->pivot->status === 'depleted' ? 'table-secondary' : '' }}">
                                                    <td>
                                                    ...
                                                    <td>
                                                        @if ($entry->pivot->reference_number)
                                                            <span
                                                                class="badge bg-primary">{{ $entry->pivot->reference_number }}</span>
                                                    ...
                                                    <td>
                                                        ...
                                                        <small
                                                            class="text-muted">{{ $material->unit_of_measurement }}</small>
                                                    </td>
                                                    <td>
                                                        <strong
                                                            class="text-success">{{ $entry->pivot->remaining_quantity ?? $entry->pivot->quantity }}</strong>
                                                        <small
                                                            class="text-muted">{{ $material->unit_of_measurement }}</small>
                                                    </td>
                                                    <td>
                                                        <span
                                                            class="text-danger">{{ $entry->pivot->total_used ?? 0 }}</span>
                                                        <small
                                                            class="text-muted">{{ $material->unit_of_measurement }}</small>
                                                    </td>
                                                    <td>
                                                        @if ($entry->pivot->unit_price)
                                                            <span
                                                                class="text-success fw-bold">${{ number_format($entry->pivot->unit_price, 2) }}</span>
                                                        @else
                                                    ...
                                                    <td>
                                                        @if ($entry->pivot->current_total_value)
                                                            <span
                                                                class="text-primary fw-bold">${{ number_format($entry->pivot->current_total_value, 2) }}</span>
                                                        @else
                                                    ...
                                                    <td>
                                                        @if ($entry->pivot->expiry_date)
                                                            <small
                                                                class="{{ \Carbon\Carbon::parse($entry->pivot->expiry_date)->isPast() ? 'text-danger' : 'text-muted' }}">
                                        ...
                                        <tfoot class="table-light">
                                            <tr>
                                                ...
                                                <td><strong>{{ $totalOriginal }}
                                                        {{ $material->unit_of_measurement }}</strong></td>
                                                <td><strong class="text-success">{{ $totalRemaining }}
                                                        {{ $material->unit_of_measurement }}</strong></td>
                                                <td><strong class="text-danger">{{ $totalUsed }}
                                                        {{ $material->unit_of_measurement }}</strong></td>
                    ...
                    @foreach ($materialEntries as $index => $entry)
                        <!-- Use Material Modal -->
                        <div class="modal fade" id="useMaterialModal{{ $entry->id }}_{{ $index }}"
                            tabindex="-1">
                                ...
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        <div class="alert alert-info">
                                            ...
                                            {{ $entry->pivot->reference_number ?? 'No Reference' }}<br>
                                            <strong>Available:</strong>
                                            {{ $entry->pivot->remaining_quantity ?? $entry->pivot->quantity }}
                                            {{ $material->unit_of_measurement }}
                                        </div>
                                        <form
                                            action="{{ route('stocks.useMaterial', [$stock->id, $entry->id, $entry->pivot->id ?? 0]) }}"
                                            method="POST">
                                            @csrf
                                            <div class="row">
                                                <div class="col-md-6">
                                                    ...
                                                    <input type="number" name="quantity_used" class="form-control"
                                                        min="1"
                                                        max="{{ $entry->pivot->remaining_quantity ?? $entry->pivot->quantity }}"
                                                        required>
                                                </div>
                                                <div class="col-md-6">
                        ...
                        <div class="modal fade" id="viewMovementModal{{ $entry->id }}_{{ $index }}"
                            tabindex="-1">
                                            ...
                                            <div class="timeline">
                                                @foreach ($movements as $movement)
                                                    <div class="timeline-item">
                                                        <div
                                                            class="timeline-marker bg-{{ $movement['type'] === 'initial_stock' ? 'success' : 'warning' }}">
                                                        </div>
                                                        <div class="timeline-content">
                                                            <h6>{{ ucfirst(str_replace('_', ' ', $movement['type'])) }}
                                                            </h6>
                                                            <p class="mb-1">
                        ...
                        <!-- Reference Modal -->
                        <div class="modal fade" id="referenceModal{{ $entry->id }}_{{ $index }}"
                            tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title">Reference Number: {{ $entry->pivot->reference_number }}
                                        </h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                                    </div>
                                    <div class="modal-body">
                        ...
                        <div class="modal fade" id="removeMaterialModal{{ $entry->id }}_{{ $index }}"
                            tabindex="-1">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                    ...
                                    <div class="modal-body">
                                        <div class="alert alert-warning">
                                            ...
                                            remove this material entry from the stock.
                                        </div>
                                        <div class="alert alert-info">
                                            <strong>Material:</strong> {{ $material->name }}<br>
                                            <strong>Reference:</strong>
                                            ...
                                            {{ $entry->pivot->remaining_quantity ?? $entry->pivot->quantity }}
                                            {{ $material->unit_of_measurement }}<br>
                                            <strong>Current Value:</strong>
                                            ${{ number_format($entry->pivot->current_total_value, 2) }}
                                        </div>
                                        <form
                                            action="{{ route('stocks.removeMaterial', [$stock->id, $entry->id, $entry->pivot->id ?? 0]) }}"
                                            method="POST">
                                            @csrf
                                            <div class="mb-3">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('stocks.addMaterial', $stock->id) }}" method="POST" id="addMaterialForm">
                        ...
                        <div class="alert alert-info">
                            <i class="fas fa-info-circle"></i>
                            <strong>Reference Numbers:</strong> Each material will automatically get a unique reference
                            number when added to stock
                        </div>
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <div class="mt-3 d-flex justify-content-between align-items-center">
                                    ...
                                    <div>
                                        <small class="text-muted">
                                            Showing <span id="visibleCount">{{ $materials->count() }}</span> of
                                        ...
                                        <div class="card border h-100 material-card">
                                            <div class="card-body p-3">
                                                ...
                                                <div class="d-flex justify-content-between align-items-start mb-2">
                                                    ...
                                                    <div class="text-end">
                                                        ...
                                                        @if ($material->category)
                                                            <span
                                                                class="badge bg-light text-dark small">{{ $material->category }}</span>
                                                        @endif
                                                    </div>
                                                </div>
                                                ...
                                                <div class="small text-muted mb-2">
                                                    <div><i
                                                            class="fas fa-ruler me-1"></i>{{ $material->unit_of_measurement }}
                                                    </div>
                                                    @if ($material->description)
                                                        <div class="mt-1">{{ Str::limit($material->description, 60) }}
                                                        </div>
                                                    @endif
                                                </div>

                                                <!-- Pricing Fields (Hidden by default) -->
                                                <div id="fields{{ $material->id }}" class="material-fields"
                                                    style="display: none;">
                                                    <hr class="my-2">
                                                    <div class="row g-2">
                                                        <div class="col-6">
                                                            <label class="form-label small fw-bold">Quantity <span
                                                                    class="text-danger">*</span></label>
                                                            <input type="number" name="quantities[{{ $material->id }}]"
                                                                class="form-control form-control-sm" placeholder="Qty"
                                                                min="1" step="1"
                                                                onchange="calculateTotal({{ $material->id }})">
                                                            <small
                                                                class="text-muted">{{ $material->unit_of_measurement }}</small>
                                                        </div>
                                                        <div class="col-6">
                                                            <label class="form-label small fw-bold">Unit Price <span
                                                                    class="text-danger">*</span></label>
                                                            <div class="input-group input-group-sm">
                                                                <span class="input-group-text">$</span>
                                                                <input type="number"
                                                                    name="unit_prices[{{ $material->id }}]"
                                                    ...
                                                    <div class="row g-2 mt-1">
                                                        ...
                                                        <div class="col-6">
                                                            ...
                                                            <input type="text"
                                                                name="batch_numbers[{{ $material->id }}]"
                                                    ...
                                                    <div class="mt-2">
                                                        ...
                                                        <textarea name="notes[{{ $material->id }}]" class="form-control form-control-sm" rows="2"
                            ...
                            <div class="reference-list" style="max-height: 300px; overflow-y: auto;">
                                ...
                                @if ($uniqueReferences->isNotEmpty())
                                    @foreach ($uniqueReferences as $reference)
                                        @php
                                            $refMaterials = $stock->materials->where(
                                                'pivot.reference_number',
                                                $reference,
                                            );
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <div>
                                                        ...
                                                        <small class="text-muted d-block">
                                                            {{ $totalMaterials }}
                                                            material{{ $totalMaterials > 1 ? 's' : '' }}
                                                            | ${{ number_format($totalValue, 2) }}
                                                        </small>
                                                        <small class="text-muted">
                                                            Added:
                                                            {{ $firstMaterial->pivot->created_at ? \Carbon\Carbon::parse($firstMaterial->pivot->created_at)->format('M d, Y') : 'N/A' }}
                                                        </small>
                                                    </div>
        ...
        if (form) {
            form.addEventListener('submit', function(e) {
                ...
                checkedBoxes.forEach(function(checkbox) {
                    ...
                    const materialName = document.querySelector(
                        `label[for="material${materialId}"]`).textContent.trim();
                    const quantity = document.querySelector(
                        `input[name="quantities[${materialId}]"]`).value;
                    const unitPrice = document.querySelector(
                        `input[name="unit_prices[${materialId}]"]`).value;
        ...
        if (selectedCount === 0) {
            ...
            if (materialNames.length <= 3) {
                ...
                summaryElement.textContent =
                    `Selected: ${materialNames.slice(0, 3).join(', ')} and ${selectedCount - 3} more`;
            }
        }
    }
